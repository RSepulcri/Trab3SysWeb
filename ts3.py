...
# Função para manipular cliques
def interactive_grid(image, evt: gr.SelectData):
    global grid, start_pos
    x, y = evt.index  # Obtém as coordenadas clicadas
    grid = process_click(image, start_pos, x, y)
    return grid
...
with gr.Blocks() as demo:
    grid_image = gr.Image(value=grid, label="Clique em um pixel!")
    btn_reset = gr.Button("Resetar")
    grid_image.select(interactive_grid, inputs=[grid_image], outputs=grid_image)
